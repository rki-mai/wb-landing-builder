...
1. Система должна предоставлять REST API для управления лендингами.
2. Система должна поддерживать создание нового лендинга.
3. Система должна поддерживать получение информации о лендинге по идентификатору.
4. Система должна поддерживать обновление данных лендинга.
5. Система должна поддерживать удаление лендинга.
6. Данные лендингов должны храниться в базе данных MongoDB.

### Нефункциональные требования

1. API должен возвращать ответы в формате JSON.
2. Архитектура приложения должна поддерживать расширение функциональности (например добавление редактора страниц).
3. Кодовая база должна быть разделена на логические модули (models, repository, db).
