...
| `/*` | статические файлы UI из `docker/static/` |

Bucket `publications` настроен на anonymous download (только GET объектов).

### Статические файлы UI

Для локальной разработки фронтенда в `docker/static/` лежит placeholder `index.html`. Собранный фронтенд можно смонтировать в CDN-контейнер вместо этой директории.

При запуске backend напрямую (`go run main.go`, без nginx) отдача статики включается через переменную `STATIC_FILES_DIR`. Если директория не задана или отсутствует, handler не регистрируется.
